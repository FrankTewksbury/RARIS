...
- [ ] **Fix checkpoint not firing in `run_resumed()`** — `_write_checkpoint()` is called at `entity_n % 50 == 0` inside `run_resumed()` but never writes to DB. Root cause: `self.db` session in `_run_agent_resumed()` is a single `async with async_session()` context shared across the entire L2 loop. After many source/program writes and commits, the `Manifest` ORM object becomes stale/expired in the session. `db.get(Manifest, id)` inside `_write_checkpoint` either returns a detached instance or the commit silently no-ops. Fix: open a **fresh short-lived session** inside `_write_checkpoint` instead of reusing `self.db`, OR call `await self.db.refresh(manifest)` before the checkpoint commit. See `backend/app/agent/graph_discovery.py` line ~1498 and `backend/app/routers/manifests.py` line ~704. `#status/backlog #priority/critical #source/session #agent/cursor`
- [ ] **Citation normalization batch pass** — after insurance run completes, run batch LLM lookup (~20 sources/call) to resolve official short-form citation identifiers (e.g. `N.J.S.A. 17:17`) for all `sources` rows where `citation` is descriptive rather than formal; update `sources.citation` in place; sources with no formal citation get `NULL`. See `context/012-note-citation-normalization-cleanup.md` `#status/backlog #priority/important #source/session #agent/cursor`
